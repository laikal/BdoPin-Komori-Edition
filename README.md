...
On supported Intel hybrid processors, BdoPin can also use detected processor topology information when building affinity presets.
...
Depending on the CPU architecture, game workload, and Windows scheduler behavior, some systems may perform more consistently when Black Desert is restricted to selected logical processors instead of using every available processor thread.
...
Results vary depending on the CPU, system configuration, game workload, and Windows scheduler behavior.
...
If automatic application is enabled, BdoPin can detect `BlackDesert64.exe` when the game starts and apply the selected configuration automatically when permission allows.
...
If elevation is required, BdoPin requests it only for the explicit user action.

Automatic background detection does not display a UAC prompt by itself.

If automatic application cannot proceed because additional permission is required, you can use **Apply Now** manually.

---

## Diagnostic Log

BdoPin creates a lightweight event-based log named:

`BdoPin.log`

The log may contain technical information useful for troubleshooting, including:

- BdoPin version and build
- CPU vendor and topology
- selected CPU preset
- selected logical processors
- process priority
- ProcessWatcher status
- Black Desert process detection
...
When the size limit is reached, the old log is removed and a new log is created automatically.

The log is intended for troubleshooting and GitHub issue reports and does not intentionally record Windows user names, PC names, account information, or unrelated personal data.
...
- patch the game
- modify the system-wide SMT / Hyper-Threading configuration
- attempt to bypass anti-cheat systems

BdoPin only uses standard Windows process-management functions to configure CPU affinity and process priority.
...
지원되는 Intel 하이브리드 CPU에서는 감지된 프로세서 토폴로지 정보를 이용하여 P-Core / E-Core 구조를 고려한 프리셋을 구성할 수 있습니다.
...
CPU 구조, 게임의 작업 부하, Windows 스케줄러의 동작 방식에 따라 일부 시스템에서는 모든 논리 프로세서를 사용하는 것보다 선택된 논리 프로세서 위주로 검은사막을 실행했을 때 더 안정적인 결과를 보일 수 있습니다.
...
결과는 CPU, 시스템 구성, 게임 작업 부하 및 Windows 스케줄러의 상태에 따라 달라질 수 있습니다.

BdoPin은 **FPS 상승이나 성능 향상을 보장하지 않습니다.**

여러 프리셋을 직접 적용해 보고 자신의 시스템에서 결과를 비교하여 사용하는 것을 권장합니다.
...
자동 적용 기능을 사용하는 경우 BdoPin은 `BlackDesert64.exe` 실행을 감지하여 권한이 허용되는 범위에서 선택한 설정을 자동으로 적용할 수 있습니다.
...
관리자 권한이 필요한 경우 BdoPin은 사용자가 직접 실행한 작업에 대해서만 권한 상승을 요청합니다.

백그라운드에서 검은사막을 자동으로 감지했다는 이유만으로 UAC 창을 자동으로 표시하지 않습니다.

자동 적용 과정에서 권한이 부족한 경우에는 **지금 적용** 기능을 이용하여 수동으로 적용할 수 있습니다.

---

## 진단 로그

BdoPin은 문제 확인을 위한 가벼운 이벤트 기반 로그를 생성합니다.

로그 파일명:

`BdoPin.log`

로그에는 문제 해결에 필요한 다음과 같은 기술 정보가 기록될 수 있습니다.
...
- 검은사막 프로세스 감지
- Affinity 적용 결과
- 작업 실패 시 Windows 오류 코드

로그는 관련 이벤트가 발생할 때만 기록됩니다.
...
로그는 문제 해결 및 GitHub Issue 확인을 위한 용도이며, Windows 사용자 이름, PC 이름, 계정 정보 등 불필요한 개인정보를 의도적으로 기록하지 않습니다.
...
- 커널 드라이버 설치
- 게임 코드 패치
- 시스템 전체의 SMT / Hyper-Threading 설정 변경
- 안티치트 우회 시도

BdoPin은 Windows의 표준 프로세스 관리 기능을 이용하여 CPU Affinity와 프로세스 우선순위를 설정하는 기능만 수행합니다.
